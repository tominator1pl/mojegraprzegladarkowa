...
if(isset($_POST['walcz'])){
    if($_POST['selectedIdPos'] >= 0) {
        if($_POST['selectedIdPos'] != $_SESSION['id_player']){
            $p = polacz();
            $zapytanie = "SELECT * FROM players WHERE ID_Player = ".$_POST['selectedIdPos'].";"; //pobieranie ofiary
            $res = $p->query($zapytanie);
            $row = $res->fetch_assoc();
            if($row){
                $zapytanie2 = "SELECT * FROM players WHERE ID_Player = ".$_SESSION['id_player'].";"; //pobieranie atakujacego
                ...
                if($row2) {
                    ...
                    if ($row['Weapon']) {
                        $zapytanieBron = "SELECT * FROM items WHERE ID_Item=" . getFromDB($p, 'inventory', 'ITEMS_ID', 'ID_Inv', $row['Weapon']) . ";";//bron ofiary
                        $resBron = $p->query($zapytanieBron);
                        $rowBron = $resBron->fetch_assoc();
                        $przedBron = itemReslove($rowBron['TYPE_ID'], $rowBron['Vars']);
                        $silBron = floor(($przedBron['param2']/10)*$przedBron['param1']);
                        $kodBron = ($row['Strength'] + $silBron);
                    }
                    if ($row['Armor']) {
                        $zapytanieArmor = "SELECT * FROM items WHERE ID_Item=" . getFromDB($p, 'inventory', 'ITEMS_ID', 'ID_Inv', $row['Armor']) . ";";//zbroja ofiary
                    ...
                    if ($row2['Weapon']) {
                        $zapytanieBron2 = "SELECT * FROM items WHERE ID_Item=" . getFromDB($p, 'inventory', 'ITEMS_ID', 'ID_Inv', $row2['Weapon']) . ";";//bron atakujaceg
                        $resBron2 = $p->query($zapytanieBron2);
                        $rowBron2 = $resBron2->fetch_assoc();
                        $przedBron2 = itemReslove($rowBron2['TYPE_ID'], $rowBron2['Vars']);
                        $silBron2 = floor(($przedBron2['param2']/10)*$przedBron2['param1']);
                        $kodBron2 = ($row2['Strength'] + $silBron2);
                    }
                    if ($row2['Armor']) {
                        $zapytanieArmor2 = "SELECT * FROM items WHERE ID_Item=" . getFromDB($p, 'inventory', 'ITEMS_ID', 'ID_Inv', $row2['Armor']) . ";";//zbroja atakujacego
                    ...
                    $kolej = 2;
                    if($kodBron2 >= $kodBron){ //test kto zaczyna atakw zaleznosci od sily obu stron
                        $kolej = 2;
                    }else{
                        $kolej = 1;
                    }
                    while($zycie > 0 && $zycie2 >0){ //sama walka
                        switch($kolej){
                            case 1:
                                $eff = floor(($kodBron+(rand(1,$kodBron/2)))-$kodArmor2); //obliczenie effektywnosci ataku
                                if($eff <= 0) $eff = 1 + floor(rand(1,$kodBron/4));//min efektywnosc
                                $zycie2 -= $eff; //cios
                                $kolej = 2; //zmiana kolejki
                                break;
                            case 2:
                    ...
                    if($zycie2 >= $zycie){
                        $nowyhais = floor($hais*(rand(1,5)/10)); //liczenie wygranej
                        $kasa = $hais2 + $nowyhais;
...
INSERT INTO komunikaty (PLAYER_ID, Komunikat, Kom_Typ_ID, Data_Kom) VALUES ('" . $_POST['selectedIdPos'] . "', 'Zostałeś zaatakowany przez " . $row2['Name_Player'] . ". Straty: Haisy-" . $nowyhais . "', '1', '" . $data . "');";
                        $res3 = $p->multi_query($zapytanie3);//AKTUALIZACJA WSZYSTKIEGO
                        if($res3){
                            rozlacz($p);
